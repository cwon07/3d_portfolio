...
import tokyoScene from "../assets/3D/littlest_tokyo.glb";
...
export function Tokyo({
  ...
  const handlePointerMove = (event) => {
    ...
    if (isRotating) {
      ...
      // Update the tokyo's rotation based on the mouse/touch movement
  ...
  useFrame(() => {
    ...
    if (!isRotating) {
      ...
      // When rotating, determine the current stage based on tokyo's orientation
      const rotation = tokyoRef.current.rotation.y;

      const normalizedRotation =
        ((rotation % (2 * Math.PI)) + 2 * Math.PI) % (2 * Math.PI);

      // Set the current stage based on tokyo's orientation
      switch (true) {
